...
namespace ExtractTransactionID
{
    class Program
    {
        ...
        /// </summary>
        /// <param name="args">inputDate, outputDate</param>
        static void Main(string[] args)
        {

            if(args.Length == 0)
            {
                inputDate = DateTime.Now.AddDays(-1);
            }
            else
            {
                string stringDate = args[0];
                inputDate = DateTime.ParseExact(stringDate, "dd/MM/yyyy", CultureInfo.InvariantCulture);
            }

            PrintHelper.Trace(Messages.NewSpace);
